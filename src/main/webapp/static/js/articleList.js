...
$(document).ready(function() {
	...
	// 填充文本框
	showInput('#articleList_queryTitle');
	showInput('#articleList_queryCreateTimeStart');
	showInput('#articleList_queryCreateTimeEnd');

	// 初始化页面上面的按钮事件
	$("#btn_add").click(function() {
		window.location.href = path + 's/article/articleAddPage';
	});
...
function saveSearchParam() {
	saveInStorage("#articleList_queryTitle", $("#articleList_queryTitle").val());
	saveInStorage("#articleList_queryCreateTimeStart", $(
			"#articleList_queryCreateTimeStart").val());
	saveInStorage("#articleList_queryCreateTimeEnd", $(
			"#articleList_queryCreateTimeEnd").val());
}
function clearSearchParam() {
	storage.clear();
	$("#articleList_queryTitle").val('');
	$("#articleList_queryCreateTimeStart").val('');
	$("#articleList_queryCreateTimeEnd").val('');
}
...
var TableInit = function() {
	...
	oTableInit.Init = function() {
		$('#table_articleList').bootstrapTable({
			url : path + 's/article/articleList.do', // 请求后台的URL（*）
			method : 'get', // 请求方式（*）
			...
			columns : [ {
				...
				formatter : function(value, row, index) {  
	                //return index + 1;  
	                var page = $('#table_articleList').bootstrapTable("getPage");  
	...
	oTableInit.queryParams = function(params) {
		var temp = { // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			...
			title : getFromStorage('#articleList_queryTitle'),
			createTimeStart_str : getFromStorage('#articleList_queryCreateTimeStart'),
			createTimeEnd_str : getFromStorage('#articleList_queryCreateTimeEnd')
		};
		return temp;
	};
	return oTableInit;
