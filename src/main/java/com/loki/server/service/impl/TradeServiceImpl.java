		...
		try {
			if (tradeVO.getTrade() != null) {
				...
				if (trade.getId() > 0) {
					...
					addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "您发布了一条贸易，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
...
//						phone.replaceAll("(\\d{3})\\d{4}(\\d{4})","$1****$2");
			...
			if (trade != null) {
				if (trade.getStatus().equals("trade_verify")) {
					...
					if (verifyResult.equals("verify_pass")) {
						...
						//已下架，清除收藏信息
						userCollectionDao.deleteByTradeId(tradeId);
					}
					if (tradeDao.update(trade)) {
						
						List<Integer> userNoticeIds=new ArrayList<>();
						userNoticeIds.add(trade.getUserId());
						addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), noticeContent, trade.getId(),userNoticeIds);
				...
				if (trade != null) {
					if (trade.getStatus().equals("trade_under_carriage")) {
						...
						if (tradeDao.update(trade)) {
							...
							List<Integer> userNoticeIds=new ArrayList<>();
							userNoticeIds.add(trade.getUserId());
							addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "贸易已上架，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
				...
				if (trade != null) {
					if (trade.getStatus().equals("trade_tendering")) {
						...
						if (tradeDao.update(trade)) {
							...
							List<Integer> userNoticeIds=new ArrayList<>();
							userNoticeIds.add(trade.getUserId());
							addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "贸易被下架了，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
				...
				if (trade != null) {
					...
					if (trade.getStatus().equals("trade_docking") && tradeDocking!=null) {
						...
						if (result) {
							...
							addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "发布方取消了贸易，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
							returnValue.setResultCode(ResultCodeEnums.SUCCESS);
						} else {
							returnValue.setResultCode(ResultCodeEnums.UPDATE_FAIL);
						}
					...
					if (tradeDocking != null && tradeDocking.getUserId() == userId) {
						if (trade.getStatus().equals("trade_docking")) {
							...
							if (result) {
								...
								userNoticeIds.add(tradeDocking.getUserId());
								userNoticeIds.add(trade.getUserId());
								addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "对接方取消了贸易，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
								returnValue.setResultCode(ResultCodeEnums.SUCCESS);
							} else {
								returnValue.setResultCode(ResultCodeEnums.UPDATE_FAIL);
							}
				...
				if (trade != null && tradeDocking != null) {
					if (trade.getStatus().equals("trade_3rd_cancel") || trade.getStatus().equals("trade_own_cancel")) {
						...
						if (result) {
							...
							addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "贸易已取消，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
				...
				if (trade != null) {
					...
					if (trade.getStatus().equals("trade_docking")) {
						...
						if (result) {
							...
							userNoticeIds.add(trade.getUserId());
							userNoticeIds.add(tradeDocking.getUserId());
							addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "发布方完成了贸易对接，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
							returnValue.setResultCode(ResultCodeEnums.SUCCESS);
						} else {
							returnValue.setResultCode(ResultCodeEnums.UPDATE_FAIL);
						}
					...
					if (tradeDocking != null && tradeDocking.getUserId() == userId) {
						if (trade.getStatus().equals("trade_docking")) {
							...
							if (result) {
								...
								userNoticeIds.add(trade.getUserId());
								userNoticeIds.add(tradeDocking.getUserId());
								addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "对接方完成了贸易对接，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
								returnValue.setResultCode(ResultCodeEnums.SUCCESS);
							} else {
								returnValue.setResultCode(ResultCodeEnums.UPDATE_FAIL);
							}
				...
				if (trade != null && tradeDocking != null) {
					if (trade.getStatus().equals("trade_3rd_success")
						...
						if (result) {
							...
							userNoticeIds.add(trade.getUserId());
							userNoticeIds.add(tradeDocking.getUserId());
							addNotice(trade.getType().equals("trade_demand") ? 2 : 5,trade.getTitle(), "贸易对接已成功，当前状态【"+noticeTradeStatus+"】", trade.getId(),userNoticeIds);
	...
	@Override
	public ServiceResult<PagedResult<TradeComplex>> getSuccessedList(Map<String, Object> map) {
		ServiceResult<PagedResult<TradeComplex>> returnValue = new ServiceResult<>();
		try {
			if (map != null) {
				int pageNo = map.get("pageNo") == null ? 1 : (int) map.get("pageNo");
				...
				PageHelper.startPage(pageNo, pageSize);
				PagedResult<TradeComplex> tradeComplexList = BeanUtil
						.toPagedResult(tradeComplexDao.findSuccessTradeList(map));
				if (tradeComplexList != null) {
					// 字段code处理
					for (int i = 0; i < tradeComplexList.getRows().size(); i++) {
						tradeComplexList.getRows().get(i).setStatusName(
						...
						tradeComplexList.getRows().get(i).setIdentityName(getIdentityName(tradeComplexList.getRows().get(i).getIdentityId()));
						tradeComplexList.getRows().get(i).setEnterpriseName(getEnterpriseName(tradeComplexList.getRows().get(i).getEnterpriseId()));
						if (tradeComplexList.getRows().get(i).getDockingId() > 0) {
							TradeDocking tradeDocking = tradeDockingDao
									.findById(tradeComplexList.getRows().get(i).getDockingId());
							if (tradeDocking != null) {
								tradeComplexList.getRows().get(i)
